...
- Alguien puede no hacer un update y build local antes de hacer commit
- Los desarrolladores pueden tener configuraciones diferentes en sus máquinas
- Así que hay que hacer los build en una máquina compartida

- Opciones: build manual o servidor de integración
...
- Servidores de CI (algunos solo disponibles en la nube): Jenkins, Gitlab CI/CD, Teamcity, Bamboo, GitHub Actions, Azure DevOps services, CircleCI, Semaphore, etc.
...
- Prioridad 1 es arreglar un build que falla
- No todo el mundo tiene que dejar de hacer lo que está haciendo para arreglarlo. Con un par de personas suele ser suficiente
...
- ¿Cuánto? ¿1 hora es mucho? ¿10 minutos?
...
Los desarrolladores deben integrar todo su trabajo en la versión principal del código (conocida como trunk, main o mainline) de forma regular, una vez al día al menos.

La CI también incluye dos prácticas más, según Kent Beck y la comunidad XP:

- La práctica de Trunk-Based Development (TBD), en la que los desarrolladores trabajan sobre el trunk en pequeños lotes y fusionan su trabajo en un trunk compartido al menos una vez al día, en lugar de trabajar en ramas de features de larga duración.

- La creación de suites de pruebas unitarias automatizadas mantenibles es compleja. Una manera de resolver este problema es practicar el TDD. Los desarrolladores escriben pruebas automatizadas que inicialmente fallan, antes de implementar el código que hace que las pruebas pasen.
...
Este proceso da como resultado un desarrollo y entrega de software más rápido y estable cuando los cambios son pequeños y autocontenidos, y las ramas en las que viven son de corta duración.
...
El CDEP es cuando los equipos intentan desplegar en producción cada cambio en el código lo antes posible. Cada cambio que pasa por el pipeline se pone automáticamente en producción, lo que resulta en muchos cambios en producción todos los días. La entrega continua (CD) significa que se pueden hacer despliegues frecuentes, pero se puede elegir no hacerlo. Para hacer despliegue continuo (CDEP) se debe hacer entrega continua (CD).
